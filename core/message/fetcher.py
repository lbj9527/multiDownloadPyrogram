"""
消息获取器
从test_downloader_stream.py提取的消息获取逻辑
"""
import asyncio
...
class MessageFetcher(LoggerMixin):
    ...
    async def parallel_fetch_messages(
        ...
        # 按消息ID排序确保顺序正确，同时过滤掉无效消息（与原程序一致）
    ...
    async def fetch_message_range(
        ...
        """
        获取指定范围的消息 - 使用与原程序完全一致的批量获取逻辑
        从test_downloader_stream.py提取的逻辑
        """
        # 错开启动时间避免同时发起请求（与原程序一致）
        if client_index > 0:
        ...
        messages = []
        batch_size = 100  # 每批获取100条消息（与原程序一致）
